...
for sqlfile in "$(dirname $0)"/*.sql; do
    dbname=$(basename "$sqlfile" .sql)
    /opt/mssql-tools/bin/sqlcmd -S "$DB_HOST" -U sa -P "$SA_PASSWORD" -i "$(dirname $0)"/"$dbname".sql
done
...
#             break # exit for loop
#         # if statement for importing user related code
#         elif [ $dbname = *"User" ]; then
#             echo ""
#             echo "-----------------------------------------------"
#             echo "- $i: importing script '$dbname.sql'          -"
#             echo "-----------------------------------------------"
#             /opt/mssql-tools/bin/sqlcmd -S "$DB_HOST" -U sa -P "$SA_PASSWORD" -i "$(dirname $0)"/"$dbname".sql
